...
export async function runExperimentalClaudeMd(
  ...
  const targetFile = options.outputFile || "CLAUDE.md";
  const claudeMdPath = join(cwd, targetFile);
  ...
  const injectSpinner = ora(`Injecting into ${targetFile}...`).start();
  ...
  if (isNewFile) {
    injectSpinner.succeed(chalk.green(`Created ${targetFile} with documentation index`));
  } else if (hasExistingIndex(existingContent)) {
    injectSpinner.succeed(chalk.green(`Updated existing documentation index in ${targetFile}`));
  } else {
    injectSpinner.succeed(chalk.green(`Appended documentation index to ${targetFile}`));
  }
  ...
  console.log(`   ${chalk.gray(`${targetFile} path:`)} ${claudeMdPath}`);
  ...
  console.log(chalk.green(`✨ Done! Your ${targetFile} now includes the Next.js documentation index.\n`));
}
