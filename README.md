...
| [`instant-nav`](skills/instant-nav/SKILL.md)                   | Fix, not just diagnose: make one navigation (or hard load) instant under Cache Components by formulating it as a red `@next/playwright` `instant()` e2e and working it to green; the shipped test then guards against regression. Gates protect against tests that lie. A setup phase discovers your project's own build/deploy/test rig — Vercel, generic CI, or local-only. Includes a [case study](skills/instant-nav/CASE-STUDY.md) of an agent driving this loop unattended against CI. |
...
- `instant-nav` only: Playwright + `@next/playwright`, and a prod build
  with `experimental.exposeTestingApiInProductionBuild` (no
  agent-browser needed).
